<template>
  <el-dialog v-model="visible" title="编辑目录" :close-on-click-modal="false" @closed="close" width="680">
    <el-form ref="editDirForm" :model="formData" label-width="100px" label-position="right">
...
export default {
  ...
  methods: {
    open(dirId, projectId, callback){
      ...
      this.getDirs()
      this.$fly.get("/api/projects/" + this.projectId + "/directories/"+dirId).then((res) => {
        this.formData = {
          id: res.data.id,
    ...
    save(){
      this.$refs["editDirForm"].validate((valid)=> {
        if (valid) {
          this.$fly.patch("/api/projects/" + this.projectId + "/directories/"+this.formData.id, this.formData).then((res) => {
